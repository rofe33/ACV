...
ACV_newref()
{
    return calloc(1, sizeof(ACV_ref));
}
...
static bool
ACV_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
ACV_book_matches(const ACV_book *book, const char *s)
{
    return ACV_bookequal(book->name, s, false) ||
        ACV_bookequal(book->abbr, s, false) ||
        ACV_bookequal(book->name, s, true);
}

static int
ACV_book_fromname(const char *s)
{
    for (int i = 0; i < ACV_books_length; i++) {
        const ACV_book *book = &ACV_books[i];
        if (ACV_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
ACV_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
ACV_parseref(ACV_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (ACV_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = ACV_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
